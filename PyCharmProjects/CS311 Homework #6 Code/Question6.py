def get_zodiac_sign():
    # Get the user's birth month and day
    month = int(input("Enter your birth month (as a number, e.g. January = 1): "))
    day = int(input("Enter your birth day: "))

    # Determine the zodiac sign based on the birth month and day
    def find_zodiac_sign(month, day):
        if (month == 1 and day >= 20) or (month == 2 and day <= 18):
            return "Aquarius"
        elif (month == 2 and day >= 19) or (month == 3 and day <= 20):
            return "Pisces"
        elif (month == 3 and day >= 21) or (month == 4 and day <= 19):
            return "Aries"
        elif (month == 4 and day >= 20) or (month == 5 and day <= 20):
            return "Taurus"
        elif (month == 5 and day >= 21) or (month == 6 and day <= 21):
            return "Gemini"
        elif (month == 6 and day >= 22) or (month == 7 and day <= 22):
            return "Cancer"
        elif (month == 7 and day >= 23) or (month == 8 and day <= 22):
            return "Leo"
        elif (month == 8 and day >= 23) or (month == 9 and day <= 22):
            return "Virgo"
        elif (month == 9 and day >= 23) or (month == 10 and day <= 23):
            return "Libra"
        elif (month == 10 and day >= 24) or (month == 11 and day <= 21):
            return "Scorpio"
        elif (month == 11 and day >= 22) or (month == 12 and day <= 21):
            return "Sagittarius"
        elif (month == 12 and day >= 22) or (month == 1 and day <= 19):
            return "Capricorn"
        else:
            return "Invalid date"

    # Call the nested function to find the zodiac sign and print the result
    sign = find_zodiac_sign(month, day)
    print("Your zodiac sign is:", sign)


# Call the function to test it
get_zodiac_sign()
